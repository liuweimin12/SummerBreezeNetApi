...
namespace Yun.Util
{
    ...
    public abstract class AtsUtils
    {
        ...
        public static string Download(string url, string destDir)
        {
            ...
            {
                var wu = new WebUtils();
                var req = wu.GetWebRequest(url, "GET");
                ...
                {
                    throw new YunException(wu.GetResponseAsString(rsp, Encoding.UTF8));
                }
            }
            catch (WebException e)
            {
                throw new YunException("isv.file-already-download", e.Message);
            }
            return file;
        }
        ...
        {
            var destFile = Path.Combine(destDir, Path.GetFileName(gzipFile));
        ...
        public static List<string> Unzip(string zipFile, string destDir)
        {
            ...
            using (var s = new ZipInputStream(File.OpenRead(zipFile)))
            {
                ...
                while ((theEntry = s.GetNextEntry()) != null)
                {
                    ...
                    var fileName = Path.Combine(destDir, theEntry.Name);
        ...
        private static string GetFileName(string contentDisposition)
        {
            ...
            {
                throw new YunException("Invalid response header format!");
            }
        }
    }
}
